...
__all__ = ['GramMatrix', 'SegmentationLosses', 'View', 'Sum', 'Mean',
           'Normalize', 'PyramidPooling','SegmentationMultiLosses', 'SegmentationSingleLosses']
...
class SegmentationSingleLosses(CrossEntropyLoss):
    """2D Cross Entropy Loss with Multi-L1oss"""
    def __init__(self, nclass=-1, weight=None,size_average=True, ignore_index=-1):
        super(SegmentationSingleLosses, self).__init__(weight, size_average, ignore_index)
        self.nclass = nclass
    ...
    def forward(self, *inputs):
        ...
        *preds, target = tuple(inputs)
        ...
        preds1 = tuple(preds)
        #print("preds1.size: ", preds1.size())
        #print("target.size: ", target.size())
        loss1 = super(SegmentationSingleLosses, self).forward(preds1, target)
        return loss1
...
class PyramidPooling(Module):
    ...
    """
    def __init__(self, in_channels, norm_layer, up_kwargs, concat_input= True):
